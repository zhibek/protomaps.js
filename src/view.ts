...
import { Zxy, TileCache, Feature, Bbox, ZxySource, PmtilesSource } from "./tilecache";
...
export let toSources = (options:any) => {
  let optionsToView = (o:any) => {
  ...
  if (options.sources) {
    for (const [key,value] of Object.entries(options.sources)) {
      sources.set(key,optionsToView(value));
    }
  } else {
    sources.set("",optionsToView(options));
  }
  return sources;
}
